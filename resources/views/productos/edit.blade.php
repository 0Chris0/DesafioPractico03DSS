...
<head>
    ...
    <title>Editar Prenda - Tienda de Ropa</title>
...
<body class="bg-slate-50 text-slate-800 font-sans antialiased">
    ...
    <div class="max-w-3xl mx-auto px-4 py-10">
        ...
        <div class="pb-6 border-b border-slate-200">
            <h1 class="text-3xl font-bold text-slate-900 tracking-tight">Modificar Prenda #{{ $producto->id }}</h1>
            <p class="mt-2 text-sm text-slate-600">Actualiza la información del inventario. Los cambios se guardarán inmediatamente.</p>
        </div>

        <form action="{{ route('productos.update', $producto->id) }}" method="POST" class="mt-8 space-y-6 bg-white p-6 rounded-xl shadow-sm border border-slate-200">
            @csrf
            @method('PUT') 

            <div>
                <label for="nombre" class="block text-sm font-medium text-slate-700">Nombre de la Prenda o Artículo</label>
                <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $producto->nombre) }}" class="mt-1 block w-full rounded-md border-slate-300 px-3 py-2 border shadow-xs focus:border-indigo-500 focus:ring-indigo-500 text-sm">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <select name="talla" id="talla" class="mt-1 block w-full rounded-md border-slate-300 px-3 py-2 border shadow-xs focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                        ...
                        @foreach($tallas as $t)
                            <option value="{{ $t }}" {{ old('talla', $producto->talla) == $t ? 'selected' : '' }}>{{ $t }}</option>
                        @endforeach
                    </select>
                    @error('talla') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label for="color" class="block text-sm font-medium text-slate-700">Color</label>
                    <input type="text" name="color" id="color" value="{{ old('color', $producto->color) }}" class="mt-1 block w-full rounded-md border-slate-300 px-3 py-2 border shadow-xs focus:border-indigo-500 focus:ring-indigo-500 text-sm">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number" step="0.01" name="precio" id="precio" value="{{ old('precio', $producto->precio) }}" class="mt-1 block w-full rounded-md border-slate-300 px-3 py-2 border shadow-xs focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                ...
                <div>
                    ...
                    <input type="number" name="stock" id="stock" value="{{ old('stock', $producto->stock) }}" class="mt-1 block w-full rounded-md border-slate-300 px-3 py-2 border shadow-xs focus:border-indigo-500 focus:ring-indigo-500 text-sm">
            ...
            <div class="flex justify-end gap-3 pt-4 border-t border-slate-100">
                ...
                <button type="submit" class="px-4 py-2 text-sm font-semibold text-white bg-amber-500 rounded-md shadow-xs hover:bg-amber-600 cursor-pointer">
                    Actualizar Prenda
                </button>
            </div>
        </form>
